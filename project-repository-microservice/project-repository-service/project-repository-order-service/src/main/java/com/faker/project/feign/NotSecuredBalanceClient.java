...
/**
 * 用户账户服务Feign接口: contextId:client唯一id, value:调用的微服务的name.
 * NotSecuredBalanceClient: 不安全的调用, 调用失败需要抛出异常而非吞掉异常: 用于事务
 * */
@FeignClient(contextId = "NotSecuredBalanceClient", value = "project-repository-account-service")
public interface NotSecuredBalanceClient {

    @RequestMapping(value = "/project-account-service/balance/deduct-balance", method = RequestMethod.PUT)
